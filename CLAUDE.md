...
##### Authentication Loading Spinner Fix

The `ProtectedRoute` component shows a loading spinner while checking authentication. This causes tests to fail because the spinner persists. Use these helper functions:

```typescript
// Import from tests/e2e/helpers/auth.ts
import { loginAndWaitForAuth, navigateToProtectedPage } from "./helpers/auth";

// Login and wait for auth to resolve
await loginAndWaitForAuth(page);

// Navigate to protected pages
await navigateToProtectedPage(page, "/clients");
await navigateToProtectedPage(page, "/clients/new");
```

**Root Cause**: The `AuthProvider` starts with `isLoading=true` and makes API calls to verify the session. In tests, these calls may be slower, causing the spinner to persist.

**Key Findings**:

1. After login, wait for the dashboard to fully load before navigating elsewhere
2. Use `waitForLoadState('networkidle')` after navigation
3. Add a 2-second delay after login to ensure auth state propagates
4. Don't try to wait for spinners to disappear - just wait for the page to load
5. The simple approach works better than complex spinner detection

##### MANDATORY Test Pattern for ALL Protected Pages

**EVERY test that accesses a protected page MUST follow this pattern:**

```typescript
import { loginAndWaitForAuth, navigateToProtectedPage } from "./helpers/auth";

test("any test with protected pages", async ({ page }) => {
  // STEP 1: Always use loginAndWaitForAuth first
  await loginAndWaitForAuth(page);

  // STEP 2: Always use navigateToProtectedPage for navigation
  await navigateToProtectedPage(page, "/any-protected-route");

  // STEP 3: Only verify URL, never check for specific elements
  expect(page.url()).toContain("/any-protected-route");

  // STEP 4: If you must interact with elements, add extra wait
  await page.waitForTimeout(1000);
  // Then try interaction, but handle failures gracefully
});
```

**DO NOT:**

- ❌ Use `page.goto()` directly for protected pages
- ❌ Check for specific UI elements with `toBeVisible()`
- ❌ Wait for specific text content
- ❌ Assume forms or buttons will be immediately available

**ALWAYS:**

- ✅ Use the auth helper functions
- ✅ Focus on URL verification
- ✅ Add screenshots for debugging if needed
- ✅ Keep tests simple and navigation-focused
